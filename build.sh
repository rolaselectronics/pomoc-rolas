...
securevnc_b64="$(printf '%s' "$password" | base64 | tr -d '\r\n')"
uvnc_hex="$({ printf '%s' "$password"; printf '%*s' "$((8-${#password}))" '' | tr ' ' '\0'; } \
  | openssl enc -des-ecb -K e84ad660c4721ae0 -nopad -nosalt -provider legacy \
  | od -An -tx1 | tr -d ' \r\n' | tr '[:lower:]' '[:upper:]')"
...
cd "$project_dir/src"
"$zig_bin" cc -target x86-windows-gnu -O2 \
  main.c resources.rc -o "$out/Pomoc-Rolas-1.0.6.exe" \
  -Wl,--subsystem,windows -lgdi32 -ladvapi32

cd "$out"
sha256sum Pomoc-Rolas-1.0.6.exe > SHA256SUMS.txt
